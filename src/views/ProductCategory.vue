<template>
    ...
    <div class="full-box w-full" :style="{ backgroundImage: `url(${url + productName.thumbnail})` }">
      <div class="text-center text-white main-container xl:px-60 md:px-20 sm:px-10">
        ...
        <div class="btn-heading flex justify-center">
          <button type="button"
            class="bg-white w-1/6 rounded-3xl text-black p-2 flex items-center justify-center hover:bg-gray hover:bg-opacity-60"
            @click="toContactSales">
  ...
  <div v-for="(product, index) in productByCategory" :key="index"
    :style="{ backgroundColor: index % 2 === 0 ? 'white' : '#1F336D' }">
    <article class=" main-container max-w py-8 grid md:grid-cols-2 md:gap-4">
      ...
      <div class="w-3/4 md:w-auto">
        ...
        <ul class=" font-poppins list-disc text-justify text-base text-navy-prim font-normal my-0 pt-6 pb-5"
          :style="{ color: index % 2 === 0 ? '#1F336D' : 'white' }">
